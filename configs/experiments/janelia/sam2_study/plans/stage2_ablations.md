...
| **held-out mAP (final ckpt, 128 val cubes, AMG 8³ lattice)** | | | | | | baseline **0.645** (recall 0.695, mIoU 0.948); membrane-only **0.651** (recall 0.700); Stage-1 epoch-6 reference 0.624; **0-click: 0.000, zero predictions** (AMG's mask-to-mask refinement feeds a mask prompt the 0-click model never saw in training -> every candidate filtered; re-scored with `use_m2m: false`) |
