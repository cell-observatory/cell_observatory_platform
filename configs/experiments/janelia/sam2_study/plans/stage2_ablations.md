...
| **held-out mAP (final ckpt, 128 val cubes, AMG 8³ lattice)** | | | | | | baseline **0.645** (recall 0.695, mIoU 0.948); membrane-only **0.651** (recall 0.700); Stage-1 epoch-6 reference 0.624; **0-click: 0.000, zero predictions** (AMG's mask-to-mask refinement feeds a mask prompt the 0-click model never saw in training -> every candidate filtered; re-scored: filters off + no m2m -> **0.623**, recall 0.69, precision 0.99, 18k preds, IoU-head Spearman 0.74 (baseline 0.91): the masks are fine, the IoU head trained on the prompt round only scores lattice clicks below the 0.5 keep threshold) |
