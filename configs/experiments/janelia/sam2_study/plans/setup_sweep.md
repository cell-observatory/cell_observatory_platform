...
| G8 | LR sweep attempt 2 (2026-09-08, LSF) | links died at ~2 h 45 min = end of epoch 1: NCCL `_ALLGATHER_BASE` timeout on one rank; no checkpoint yet (wall-clock save 3.5 h > epoch 2.7 h) | the validation iterator collates into the training collator's device buffer while the train prefetch still holds every slot → `buffers.get_free` raises after the 600 s slot wait on that rank → the other ranks time out; the separate validation collator existed only for step-cadence validation | `data/dataloaders.py`: validation always gets its own collator when `split > 0`; `stage1_lr/lr_base.yaml`: `periodic_checkpointer.time_interval: 1h` | fixed (uncommitted); relaunch after chain_smoke |
