...
| G8 | LR sweep attempt 2 (2026-09-08, LSF) | links died at ~2 h 45 min = end of epoch 1: NCCL `_ALLGATHER_BASE` timeout on one rank; no checkpoint yet (wall-clock save 3.5 h > epoch 2.7 h) | the validation iterator collates into the training collator's device buffer while the train prefetch still holds every slot → `buffers.get_free` raises after the 600 s slot wait on that rank → the other ranks time out; the separate validation collator existed only for step-cadence validation | primary cause (found with the mini smoke + code analysis): eval-mode click sampling `pt_sampling_for_eval: center` runs two CPU distance transforms per object on full-res masks → minutes per validation step, rank-dependent → validation desyncs and every 600 s timeout fires (the lr plan's unresolved prerequisite). Fixes: recipes `pt_sampling_for_eval: uniform`; validation gets its own collator (`data/dataloaders.py`); hourly wall-clock checkpoint; start-up warning in `models/meta_arch/sam.py` for `center`; progressive slot-wait warnings in `data/datasets/buffers.py`; `chain_smoke_mini` config (1024 rows, 3 epochs, short timeouts) as the boundary regression test | fixed; relaunch after chain_smoke_mini passes a validation epoch |
