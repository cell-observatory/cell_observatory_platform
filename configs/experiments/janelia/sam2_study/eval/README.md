...
as a submitted job (no interactive allocation): `scripts/utils/submit_eval_queue.sh <run worktree> experiments/janelia/sam2_study/eval/<dir> <hours> <leaf> ...`.
...
| Stage 2 ablations (channels, dropout, clicks, gt-prob, SNR) | `stage2_ablations/<leaf>` | `stage2/eval_abl_<run>.yaml` (final checkpoints copied to `$DATA_DIR/sam2_study/eval/ckpts/stage2/<run>/checkpoints`; dropout runs scored on all channels), `stage2/eval_abl_{baseline,E_snr_*}_at_snr{5000,1500,500}.yaml` (cross-SNR matrix), `stage2/eval_sam2_tile_adapt.yaml` |
