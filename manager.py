...
def main(cfg: DictConfig):
    ...
    if cfg.run_type == "multi_run":
        assert len(list(cfg.runs)) > 0, "cfg.runs must be a list of configurations for multiple training jobs."
        ...
        raise ValueError(
            f"Unknown run type: {cfg.run_type}. "
            f"Please set cfg.run_type to either 'single_run', 'multi_run', or 'tune'."
        )


def write_chain_resubmit_script(outdir: str, bsub_args: list[str], ray_wrap: str, chain_min_runtime: int) -> Path:
    """Write ``<outdir>/chain_resubmit.sh``: the bsub command for the next link of
    an LSF job chain. ``bsub_args`` are the resource/queue/name/log arguments of
    the parent job (no -w, -env or command). The wrapper (cluster/chain_lib.sh)
    runs it with PARENT_JOBID and CHAIN_REMAINING set; the follower is held
    with -w "ended(PARENT_JOBID)" so only one job is ever pending."""
    lines = [
        "#!/usr/bin/env bash",
        "# Generated by manager.py (clusters.chain_jobs > 1). Run by cluster/chain_lib.sh",
        "# at job start to submit the next link of the chain. Not for manual use.",
        "set -euo pipefail",
        ': "${PARENT_JOBID:?PARENT_JOBID unset}" "${CHAIN_REMAINING:?CHAIN_REMAINING unset}"',
        "exec " + " ".join(bsub_args) + " \\",
        '    -w "ended(${PARENT_JOBID})" \\',
        f'    -env "all,CHAIN_REMAINING=${{CHAIN_REMAINING}},CHAIN_MIN_RUNTIME=${{CHAIN_MIN_RUNTIME:-{chain_min_runtime}}}" \\',
        "    " + q(ray_wrap),
        "",
    ]
    Path(outdir).mkdir(parents=True, exist_ok=True)
    script = Path(outdir) / "chain_resubmit.sh"
    script.write_text("\n".join(lines))
    script.chmod(0o755)
    print(f"Chain follower command written to {script}")
    return script
...
def launch_job(cfg: DictConfig, run_config_name: str = None):
    ...
    assert (cfg.paths.apptainer_image is None) != (
        cfg.paths.docker_image is None
    ...
    # LSF job chaining (clusters.chain_jobs > 1): every link of the chain runs
    # the same task with resume pointed at the run's own checkpoint dir (the
    # first link finds none and starts fresh); see cluster/chain_lib.sh.
    chain_jobs = int(cfg.clusters.get("chain_jobs", 1) or 1)
    if cfg.clusters.launcher_type == "lsf" and chain_jobs > 1:
        task += f" checkpoint.resume_run=true paths.resume_checkpointdir={outdir}/checkpoints"
    ...
    if cfg.clusters.launcher_type == "local":  # for running jobs on your local workstation without a job scheduler
        ...
        if chain_jobs > 1:
            # the follower command (same resources, held until the parent
            # ends) is run by the wrapper at every link's start
            chain_min_runtime = int(cfg.clusters.get("chain_min_runtime", 1200) or 1200)
            write_chain_resubmit_script(outdir, sjob_worker_nodes, ray_wrap, chain_min_runtime)
            env_spec = f"all,CHAIN_REMAINING={chain_jobs - 1},CHAIN_MIN_RUNTIME={chain_min_runtime}"
        else:
            env_spec = "all"

        if cfg.clusters.dependency is not None:
            sjob_worker_nodes.append(f'-w "done({cfg.clusters.job_name})"')
        sjob_worker_nodes.append(f'-env "{env_spec}"')
        ...
        if cfg.clusters.interactive_session_type:
            ray_args = [
                "bash",
                cfg.paths.ray_script,
                "-c",
                str(cfg.clusters.cpus_per_worker),
                "-g",
                str(cfg.clusters.gpus_per_worker),
                "-m",
                str(cfg.clusters.mem_per_worker),
                "-n",
                str(cfg.clusters.worker_nodes),
                "-o",
                str(outdir),
                "-q",
                str(cfg.clusters.object_store_memory),
                "-t",
                f"{task}",
            ]
            ...
            args = [
                ...
                str(cfg.clusters.worker_nodes),
                "--data",
                f"{cfg.paths.pvc_data_name}={cfg.paths.pvc_data_mount_path}",
                "--base",
                image,
                "--repo",
                cfg.clusters.repo_url,
                "--evar",
                f"NUM_NODES={cfg.clusters.worker_nodes}",
                "--evar",
                f"JOB_NAME={runai_jobname}",
                "--evar",
                f"CFG_SAVEDIR={posixify(OmegaConf.select(cfg,'paths.outdir'))}",
                "--evar",
                f"TMPDIR={posixify(OmegaConf.select(cfg,'paths.tmpdir'))}",
                "--evar",
                f"EXP_NAME={OmegaConf.select(cfg,'experiment_name')}.yaml",
                "--evar",
                f"PYTHONPATH={cfg.paths.python_path}",
                "--init",
                cfg.clusters.init_script,
                "--node-pools",
                cfg.clusters.node_pool,
                "--main",
                main_value,
            ]
