...
def encoder_transformer_flops(num_tokens, layers, embed_dim, heads, mlp_dim):
    ...
    logger.info(f"encoder transformer: {flops:,} FLOPs = {gflops} GFLOPs")
    return flops
...
def decoder_transformer_flops(num_tokens, layers, embed_dim, heads, mlp_dim):
    ...
    gflops = np.round(flops / 1e9, 3)
    logger.info(f"decoder transformer: {flops:,} FLOPs = {gflops} GFLOPs")
    return flops
...
def encoder_transformer_params(layers, embed_dim, mlp_dim):
    ...
    logger.info(f"encoder transformer: {encoder:,} params = {encoder_mparams} M params")
...
def decoder_transformer_params(layers, embed_dim, mlp_dim):
    ...
    logger.info(f"decoder transformer: {decoder:,} params = {decoder_mparams} M params")
    # feed_forward = 8 * embed_dim**2 + 5 * embed_dim
...
def transformer_inference_memory_footprint(params, dtype='float16'):
    if dtype == 'float16':
        s = 2.0
    elif dtype == 'float32':
        s = 4.0
    ...
    logger.info(f"inference memory: {mem:,d} B = {gbytes} GB using ({dtype})")
    return gbytes


def transformer_training_memory_footprint(params, dtype='float16'):
    if dtype == 'float16':
        s = 2.0
    elif dtype == 'float32':
        s = 4.0
    ...
    logger.info(f"training memory: {mem:,d} B = {gbytes} GB using ({dtype})")
    return gbytes


def data_memory_footprint(volume_size, batch_size=1, dtype='float16'):
    if dtype == 'float8':
        s = 1.0
    elif dtype == 'float16':
        s = 2.0
    ...
    logger.info(f"data memory: {mem:,d} B = {gbytes} GB using ({dtype})")
    return gbytes
...
def compute_time(flops, gpu="H100", unit="seconds", mfu=.5):
    ...
    if gpu == "TPUv3":
        ...
        peak_theoretical_flops = 123 * 10 ** 12
        ...
        peak_theoretical_flops = 312 * 10 ** 12
    ...
    effective_flops = peak_theoretical_flops * mfu
    time = flops / effective_flops
    ...
    logger.info(f"compute time: {time} {unit} using ({gpu}, mfu={mfu})")
    return time
