...
def scaling_transformer(
    ...
    for patch in [14, 16]:
        patches = {
            ...
            "2D(rgb)": {"t": 1, "z": 1, "y": patch, "x": patch, "c": ishape['c']},
            "3D(g)": {"t": 1, "z": patch, "y": patch, "x": patch, "c": 1},
            "3D(rgb)": {"t": 1, "z": patch, "y": patch, "x": patch, "c": ishape['c']},
            "4D(g)": {"t": 2, "z": patch, "y": patch, "x": patch, "c": 1},
            "4D(rgb)": {"t": 2, "z": patch, "y": patch, "x": patch, "c": ishape['c']}
        }
...
def scaling_vit(
    ...
    for patch in [14, 16]:
        ...
        for dims in vit_dimensions.keys():
            ...
            for v in vits:
                logger.info(f"{dims} ViT {v}/{patch}")
                logger.info(f"patchify: {volume_size=}, {patch_size=} -> {num_tokens:,d} tokens")
    ...
    for patch in [14, 16]:
        ...
        for dims in maes_dimensions.keys():
            ...
            for v in maes_encoders:
                logger.info(f"{dims} MAE {v}/{patch}")
                logger.info(f"patchify: {volume_size=}, {patch_size=} -> {num_encoder_tokens:,d} encoder tokens, {num_decoder_tokens:,d} decoder tokens")
    ...
    if args.arch == "powerlaw":
        ...
        for arch in df['transformer'].unique():
            ...
            vis.plot_data_parameter_scaling(
                ...
                legend=[
                    'Data (x, y, z, t, c)',
                        f'({args.ishape["x"]}, {args.ishape["y"]}, {args.ishape["z"]}, {args.ishape["t"]}, {args.ishape["c"] if args.rgb else 1})',
                        f'({args.ishape["x"]}, {args.ishape["y"]}, {args.ishape["z"]}, 1, {args.ishape["c"] if args.rgb else 1})',
                        f'({args.ishape["x"]}, {args.ishape["y"]}, 1, 1, {args.ishape["c"] if args.rgb else 1})',
                    'Patch (x, y, z, t, c)',
            ...
            #             f'({args.ishape["x"]}, {args.ishape["y"]}, {args.ishape["z"]}, {args.ishape["t"]}, {args.ishape["c"] if args.rgb else 1})',
            #             f'({args.ishape["x"]}, {args.ishape["y"]}, {args.ishape["z"]}, 1, {args.ishape["c"] if args.rgb else 1})',
            #             f'({args.ishape["x"]}, {args.ishape["y"]}, 1, 1, {3 if args.rgb else 1})',
            #         'Patch (x, y, z, t, c)',
            #             f'({args.ipatch["x"]}, {args.ipatch["y"]}, {args.ipatch["z"]}, {args.ipatch["t"]}, {args.ipatch["c"] if args.rgb else 1})',
            #         'Model FLOPs Utilization',
            #             'MFU(0.3)',
