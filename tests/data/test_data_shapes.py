...
def test_spatial_temporal_shape_4d(layout, shape_nb, shape_b, has_t):
    ...
    assert (z, y, x) == (
        shape_nb[-3 if layout.is_channel_first() else 1],
    ...
    assert (z, y, x) == (
        shape_b[-3 if layout.is_channel_first() else 2],
...
def test_permutation_roundtrip(layout, shape_nb, shape_b, has_t):
    ...
    if layout.is_channel_first():
        ...
        assert torch.equal(tensor_orig, old_tensor), (
            "Round-trip permutation did not preserve data: " f"{tensor_orig.shape} vs {old_tensor.shape}"
        )
    else:
    ...
    if layout.is_channel_first():
        ...
        assert torch.equal(tensor_orig_b, old_tensor), (
            "Round-trip permutation did not preserve data: " f"{tensor_orig_b.shape} vs {old_tensor.shape}"
        )
    else:
