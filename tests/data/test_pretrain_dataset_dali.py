...
def test_dataloader_dali(config):
    ...
    with open_dict(config):
        ...
        config.datasets.dataset._target_ = (
            "cell_observatory_platform.data.datasets.pretrain_dataset_dali.PretrainDatasetDali"
        )
    ...
    dataset = instantiate(config.datasets.dataset, batch_size=config.clusters.batch_size_per_gpu)
    ...
    for idx, data_sample in enumerate(dataloader):
        ...
        assert data_tensor.ndim == (
            dataset.input_layout.ndim + 1
        ), f"Data tensor should have {dataset.input_layout.ndim + 1} dimensions, got {data_tensor.ndim}"
        assert (
            data_tensor.shape[0] == config.clusters.batch_size_per_gpu
...
def _test_dataloader_dali_dist(config):
    ...
    for idx, data_sample in enumerate(trainer.train_dataloader):
        ...
        assert isinstance(data_tensor, torch.Tensor), "Data tensor should be a PyTorch tensor"
        assert (
            data_tensor.ndim == len(list(config.datasets.input_shape)) + 1
...
def test_data_pipeline_dali(config):
    ...
    with open_dict(config):
        ...
        config.datasets.transforms.transforms_list = [
            "cell_observatory_platform.data.transforms.normalize.NormalizeDaliWrapper"
        ]
        config.datasets.dataset._target_ = (
            "cell_observatory_platform.data.datasets.pretrain_dataset_dali.PretrainDatasetDali"
        )
    ...
    metrics = distributed_test(
        cfg=config, test="cell_observatory_platform.tests.data.test_pretrain_dataset_dali._test_dataloader_dali_dist"
    )
