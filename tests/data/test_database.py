                ...
                if hist_val is not None and pd.notna(hist_val):
                    if isinstance(hist_val, str):
                        try:
                            ...
                            assert isinstance(hist_dict, dict), f"Row {i}: {hist_col} should be a dict, got {type(hist_dict)}"
                            
                            for key, val in hist_dict.items():
                                assert isinstance(key, str), f"Row {i}: {hist_col} keys should be strings, got {type(key)}"
                                assert isinstance(val, (int, float)), f"Row {i}: {hist_col} values should be numbers, got {type(val)}"
                        except json.JSONDecodeError as e:
                            raise AssertionError(f"Row {i}: {hist_col} is not valid JSON: {e}")
                    else:
                        assert isinstance(hist_val, dict), f"Row {i}: {hist_col} should be a dict or JSON string, got {type(hist_val)}"
                ...
                if mask_val is not None and pd.notna(mask_val):
                    if isinstance(mask_val, str):
                        try:
                            ...
                            assert isinstance(mask_dict, dict), f"Row {i}: {mask_col} should be a dict, got {type(mask_dict)}"
                        except json.JSONDecodeError as e:
                            raise AssertionError(f"Row {i}: {mask_col} is not valid JSON: {e}")
                    else:
                        mask_dict = mask_val
                        assert isinstance(mask_dict, dict), f"Row {i}: {mask_col} should be a dict or JSON string, got {type(mask_dict)}"
                    
                    for cell_id, bbox in mask_dict.items():
                        assert isinstance(bbox, dict), f"Row {i}: {mask_col} cell '{cell_id}' bbox should be a dict, got {type(bbox)}"
                        required_keys = {'zmin', 'ymin', 'xmin', 'zmax', 'ymax', 'xmax'}
                        
                        assert set(bbox.keys()) == required_keys, f"Row {i}: {mask_col} cell '{cell_id}' bbox should have keys {required_keys}, got {set(bbox.keys())}"
                        for key in required_keys:
                            assert isinstance(bbox[key], (int, float)), f"Row {i}: {mask_col} cell '{cell_id}' bbox['{key}'] should be a number, got {type(bbox[key])}"