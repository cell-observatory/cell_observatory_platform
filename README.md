...
### arm64 or aarch64
First you need to build the base nvidia image for torch with the `--arch arm64` flag to create a `pytorch_25.08-py3.sif`:
```shell
apptainer pull --arch arm64 --force pytorch_25.08-py3.sif docker://nvcr.io/nvidia/pytorch:25.08-py3
```

Then, we can run the following command to create an `arm64` compatible image:
```shell
apptainer build --arch arm64 --nv --force develop_torch_cuda_12_8.sif apptainerfile.def
```
