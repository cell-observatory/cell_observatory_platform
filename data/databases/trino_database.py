...
class TrinoDatabase(ParentDatabase):
    ...
    def execute_query(self, query: str | list[str]) -> pd.DataFrame:
        ...
        if isinstance(query, list):
            assert (
                len(query) == 1
            ), f"code does not handle trino with a batch of queries and given query has length of {len(query)}, max partitions {self.max_partitions}, query {" ".join(query.split())}."
            query = query[0]
        ...
        query = query.replace("SET statement_timeout = 0;", "")  # trino does not support statement timeout setting
