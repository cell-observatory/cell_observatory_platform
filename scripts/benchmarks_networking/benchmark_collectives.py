# Copied from: https://github.com/stas00/ml-engineering/blob/master/network/benchmarks/all_reduce_bench.py
...
fmt_bytes = lambda v : str(v >> ((max(v.bit_length()-1, 0)//10)*10)) \
    +["", "K", "M", "G", "T", "P", "E"][max(v.bit_length()-1, 0)//10]+"iB"
# following the common networking hw spec convention which uses base 10, 
# instead of 2 for bps/Bps (it makes speed look bigger than it is)
...
def plot(path, x, y, ranks):
    ...
    try:
        ...
        print("!!! Can't generate plot. Please run "
...
def run(local_rank, outdir):
    ...
    plot_path = os.path.join(outdir, f"busbw-{hostname}-{ranks}.png")
    ...
    # this is useful for when one wants to interrupt the run
    # and still report the best outcome so far
...
def device_id_kwargs(local_rank):
    """
    torch.dist in recent pytorch versions loudly complains
    about device_id not being set, but it's a very problematic setting.
    this util returns a dict to be passed to `dist.init_process_group` 
    to set `device_id` if it's safe to do so.
    """
    ...
    # 2. setting device_id leads to hanging in 
    #    2.6.0<torch<2.7.1 https://github.com/pytorch/pytorch/issues/153960
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Benchmark all_reduce collective")
    parser.add_argument("--outdir", type=str, default=".", help="Output directory for the plot")
