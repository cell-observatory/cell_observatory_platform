...
from cell_observatory_platform.inference.viz_worker import VizWorker
...
class InferencerWorker:
    def __init__(
        ...
        inference_mode: Literal["sliding_window", "full_tile"],
        task: str,
        ...
        temporal_patch_size, self.axial_patch_size, self.lateral_patch_size = get_patch_sizes(
            input_format=self.input_format, patch_shape=tuple(patch_shape)
        )
    ...
    def predict(self, data_sample: dict):
        ...
        """
        if self.inference_mode == "full_tile":
    ...
    def _save_inference_outputs(self, data_sample: dict, preds: dict) -> None:
        ...
        for name, metadata in self.outputs_metadata.items():
            # TODO: See above; move heavy tensors to outer level so we don't need to search like this
            if name in preds.keys():
                output_tensor = preds[name]
            elif name in data_sample.keys():
                output_tensor = data_sample[name]
            elif name in data_sample.get("metainfo", {}).keys():
                output_tensor = data_sample.get("metainfo", {}).get(name, None)
            else:
                raise ValueError(f"Output {name} not found in preds or data_sample")
            
            if output_tensor is None:
                continue

            if metadata.get("save", False) and self.save_outputs:
                save_buffer = self.buffer_manager.get_buffer(f"{name}_save")
                if self.block_on_save:
                    slot_info = ray.get(save_buffer.get_free.remote())
                    if slot_info is None:
                        raise RuntimeError(f"No free slot found for {name}")
                else:
            ...
            if metadata.get("viz", False) and self.vizualize_outputs:
                viz_buffer = self.buffer_manager.get_buffer(f"{name}_viz")
        ...
        if self.vizualize_outputs:
            ...
            self.viz_worker.vizualize.remote(
